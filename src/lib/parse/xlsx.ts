...
import { loadScript } from '../util/cdn'
...
async function loadSheetJS(): Promise<typeof XLSXTypes> {
  if (window.XLSX) return window.XLSX
  try {
    await loadScript(CDN_URL)
  } catch {
    throw new Error('SheetJS를 CDN에서 가져오지 못했습니다. 인터넷 연결을 확인하세요.')
  }
  if (!window.XLSX) throw new Error('SheetJS를 불러왔지만 초기화에 실패했습니다')
  return window.XLSX
}
