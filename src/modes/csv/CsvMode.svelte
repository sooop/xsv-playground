<script lang="ts">
  ...
  // --- 토스트 --- (모듈 싱글턴 `toasts`에 위임 — 하위 컴포넌트는 콜백 prop으로 받는다)
...
ORD-1010,한소진,노이즈캔슬링 헤드폰,1,349000,2026-01-21,배송중`
  ...
  function openContextMenu(e: MouseEvent): void {
    ...
    menu = {
      ...
      items: [
        ...
        { label: '', sep: true },
        {
          label: hasSel ? '선택 영역을 jq로 보내기' : '표 전체를 jq로 보내기',
          accel: 'g',
          run: () => void sendToJq(hasSel ? 'selection' : 'all'),
        },
        { label: '', sep: true },
        {
          label: '열 나누기…',
  ...
  function openToolsMenu(e: MouseEvent): void {
    ...
    toolsMenu = {
      ...
      items: [
        ...
        { label: '', sep: true },
        { label: 'jq로 보내기 — 전체', accel: 'g', run: () => void sendToJq('all') },
        {
          label: 'jq로 보내기 — 보이는 부분',
          accel: 'v',
          disabled: !view.hasAnyFilter && !view.hasHidden,
          run: () => void sendToJq('view'),
        },
        {
          label: 'jq로 보내기 — 선택 영역',
          accel: 'e',
          disabled: sel.ranges.length === 0,
          run: () => void sendToJq('selection'),
        },
      ],
    }
  }
  ...
  /**
   * 표를 JSON 객체 배열로 만들어 jq 모드 입력으로 보낸다. 범위는 내보내기와 같은 3종이고,
   * 행렬은 내보내기와 **같은 함수**(`buildMatrix`)로 만든다 — 내보낸 CSV와 jq에 보낸 JSON이
   * 다른 행을 담는 일이 없어야 한다.
   */
  async function sendToJq(scope: ExportOptions['scope']): Promise<void> {
    const { matrix, headerRow } = buildMatrix(ds, view, sel, {
      scope,
      format: 'csv',
      target: 'clipboard',
      includeHeader: true,
    ...
    if (matrix.length === 0) {
      toast('보낼 행이 없습니다', 'warn')
      return
    }
    const header = headerRow ? matrix[0] : matrix[0].map((_, i) => `col${i + 1}`)
    ...
    const base = ds.fileName ? ds.fileName.replace(/\.[^.]+$/, '') : 'table'
    const sent = await shell.sendTo('jq', {
      kind: 'text',
      text: JSON.stringify(objects, null, 2),
      name: `${base}.json`,
      from: 'csv',
    })
    if (sent) toast(`${num(objects.length)}행을 jq로 보냈습니다`, 'ok')
  }
  ...
  // --- ModeHandle (셸 계약) ---
  //
  // 새 진입점은 전부 기존 `loadFiles`/`loadText`/`applyParsed` 경로를 지난다 — 이 경로를 지나야
  // `currentDoc = null`(저장 문서와의 연결 해제)과 뷰·선택·히스토리 초기화가 빠지지 않는다.
  ...
  export async function receive(p: ModePayload): Promise<void> {
    ...
    // 표는 텍스트로 직렬화하지 않고 바로 ParseResult로 만든다(인용·개행 왕복 손실 방지)
    lastBuffer = null
    encoding = 'utf-8'
    encodingBanner = false
    const result = finalize([p.header, ...p.rows], ',', true, false)
    workbook = null
    await applyParsed(result, p.name)
    toast(`${num(result.rows.length)}행 × ${result.width}열`, 'ok')
  }
  ...
  // 비활성으로 바뀌면 떠 있던 팝업을 모두 닫는다 — 숨은 페인에 메뉴가 남아 있으면 안 된다.
  // 다시 활성화되면 그리드 뷰포트를 한 번 다시 잰다(visibility 전환은 ResizeObserver를 깨우지 않는다).
  $effect(() => {
    if (isActive) {
      requestAnimationFrame(() => grid?.remeasure())
      return
    }
    menu = null
    toolsMenu = null
...
<section class="mode">
  ...
  {:else}
    <DropZone
      active={isActive}
      accept={OPEN_ACCEPT}
      onFiles={(f) => void shell.openFiles(f)}
