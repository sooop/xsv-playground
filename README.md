...
띄운다(`src/lib/ui/dialog/`). 세 모드가 이 하나를 공유한다. 그보다 큰 다이얼로그(내보내기·바꾸기·
나누기·결합·문서 목록·시트 선택·도움말·jq Transform·치트시트·스니펫·커맨드 팰릿)는 모두 공용 껍데기
`src/lib/ui/Modal.svelte` 위에 있다 — 스크림·헤더(×)·`[취소] … [확인]` 푸터·등장 애니메이션·초기
포커스·포커스 트랩·닫힌 뒤 포커스 복원을 한 곳에서 맡고, **Escape는 셸이 열린 모달 스택의 맨 위만
닫는다**. 앵커에 붙는 팝오버(열 관리·칼럼 필터·jq 히스토리 메뉴·타이포그래피)는 `dismissable()`로
Escape와 바깥 클릭을 함께 갖는다. 겹침 순서는 `app.css`의 `--z-popover < --z-menu < --z-modal <
--z-toast < --z-system-dialog` 토큰으로만 정한다.
